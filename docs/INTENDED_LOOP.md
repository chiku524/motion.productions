...
- **Renderer:** `src/procedural/renderer.py` — spec + time → one RGB frame. On pairing clips, **motion is pixel color-change**; **sync** of those changes forms objects and motion ([PIXEL_FIELD.md](PIXEL_FIELD.md)). Pipeline adds audio → MP4. Named-subject clips still use layers, camera, and lighting.
