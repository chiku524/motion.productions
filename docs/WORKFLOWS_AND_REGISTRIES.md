...
- **Creation:** Pillow **cel kit** (`src/procedural/cel.py`, `render_engine=cel`): hard ink outlines, flat fills, modern-day rooms and a TV-cartoon character rig.
- **Reference origin (optional):** ingest a clip you have rights to with `python scripts/ingest_reference.py path.mp4 --loop cartoon`. That extracts **measurements** (limited palette, ink, hold/snap timing) into the registries and a recipe at `knowledge/loop_origins/cartoon.json`. Samples span the whole clip (default 72 frames). The cartoon loop starts from that recipe. Source frames are **not** copied or replayed. Do not ingest a copyrighted show in order to reproduce it.
