...
For **pairing clips** (the default loop): the renderer changes pixel colors; it does not pan the field. Named-subject prompts (`a person walking in a forest`) still use the object path.
