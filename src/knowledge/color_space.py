...
def oklab_to_rgb(lab: Sequence[float]) -> tuple[int, int, int]:
    ...
    m = m_ * m_ * m_
    s = s_ * s_ * s_
    r = +4.0767416621 * l_cone - 3.3077115913 * m + 0.2309699292 * s
    g = -1.2684380046 * l_cone + 2.6097574011 * m - 0.3413193965 * s
    b2 = -0.0041960863 * l_cone - 0.7034186147 * m + 1.7076147010 * s
    return (
...
def _oklab_to_rgb_arrays(L, a, b):
    ...
    m = m_ * m_ * m_
    s = s_ * s_ * s_
    lr = +4.0767416621 * l_cone - 3.3077115913 * m + 0.2309699292 * s
    lg = -1.2684380046 * l_cone + 2.6097574011 * m - 0.3413193965 * s
    lb = -0.0041960863 * l_cone - 0.7034186147 * m + 1.7076147010 * s
    return (
