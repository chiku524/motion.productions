...
- Compose profiles are intentional, not accidental: default/documented fleet is `--profile free` (Core 4: explorer, balanced, interpret, sound). `exploiter` only comes up under `--profile loops-full` — it's opt-in because it adds extra D1 write pressure and should only be enabled when 500/503s stay rare (see `docs/LOCAL_COMPUTE.md`). `video-ai-render` only comes up under `--profile video-ai` and needs `OPENAI_API_KEY`/`VIDEO_AI_RENDER_SECRET` set. Seeing them absent from `docker ps` is not a bug.
